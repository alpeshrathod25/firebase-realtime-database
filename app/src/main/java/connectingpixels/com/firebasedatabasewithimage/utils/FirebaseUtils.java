package connectingpixels.com.firebasedatabasewithimage.utils;
...
import connectingpixels.com.firebasedatabasewithimage.FirebaseListeners;
...
public class FirebaseUtils {
    ...
    public void readData(final int requestCode, String dbReferenceKey,  final FirebaseListeners.OnFirebaseReadListener onFirebaseReadListener) {
        databaseReference = firebaseDatabase.getReference(dbReferenceKey);
    ...
    public void writeData(final int requestCode, String referenceKey, Object object, final FirebaseListeners.OnFirebaseWriteListener onFirebaseWriteListener) {
        databaseReference = firebaseDatabase.getReference(referenceKey);
    ...
    public void updateData(final int requestCode, String referenceKey, String taskId, Object object, final FirebaseListeners.OnFirebaseUpdateListener onFirebaseUpdateListener) {
        databaseReference = firebaseDatabase.getReference(referenceKey);
    ...
    public void deleteData(final int requestCode, String referenceKey,  String taskId, final FirebaseListeners.OnFirebaseDeleteListener onFirebaseDeleteListener) {
        databaseReference = firebaseDatabase.getReference(referenceKey);
    ...
    public void downloadFile(final int requestCode,  String directoryName, String fileName, String tempFileName, String tempFileExtension, final FirebaseListeners.OnFirebaseFileDownloadListener onFirebaseFileDownloadListener){
        try {
            downloadLocalTempFile = File.createTempFile(tempFileName, tempFileExtension);
        } catch (IOException e) {
            e.printStackTrace();
        }
        FirebaseStorage.getInstance().getReference(directoryName).child( fileName).getFile(downloadLocalTempFile).addOnSuccessListener(new OnSuccessListener<FileDownloadTask.TaskSnapshot>() {
            @Override
